...
def fast_eval_recall(db, results, proposal_nums, iou_thrs, logger=None):
    gt_bboxes = []
    for i in range(len(db.imgs.items())):
        #ann_ids = self.coco.getAnnIds(imgIds=self.img_ids[i])
        ann_ids = db.get_ann_ids(i)
        #ann_info = self.coco.loadAnns(ann_ids)
        ann_info = db.load_anns(ann_ids)
        if len(ann_info) == 0:
...
def main():
    ...
    if(opts.rpn):
        ...
        logger = None
        ar = fast_eval_recall(db,
                    results_raw, proposal_nums, iou_thrs, logger='silent')
        log_msg = []

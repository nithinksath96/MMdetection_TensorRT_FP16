...
class TwoStageDetector(BaseDetector, RPNTestMixin, BBoxTestMixin,
    ...
    def compare_fn(self,x,y):
        if(x.shape!=y.shape):
            return ("Shape did not match",x.shape,y.shape,x,y)
        for i in range(x.shape[0]):
            if(x[i]!=y[i]):
                return("Value at index did not match",i,x,y)
    ...
    def simple_test(self, img, img_meta, proposals=None, rescale=False):
        ...
        # if(proposal_list[0].shape[0]!=1000):
        #     b,p = proposal_list[0].shape
        #     batch_size_appended = 1000 - b
        #     appended_proposals = torch.zeros(batch_size_appended,p).cuda()
        #     proposal_list[0] = torch.cat((proposal_list[0],appended_proposals),dim=0)

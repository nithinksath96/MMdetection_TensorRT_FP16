...
out_name = 'mask_stitching.html'
title = 'Mask Stitching'
metric = 'Mask AP'
link_video = False
n_show = 90
n_consec = 15
...
dirs = [
    join(data_dir, 'Exp/ArgoVerse1.1/vis/pps_mrcnn50_ds_s0.5_kf_fba_iou_lin/' + split),
    join(data_dir, 'Exp/ArgoVerse1.1/vis/box_mask_stitching/' + split),
    join(data_dir, 'Exp/ArgoVerse1.1/vis/box_mask_stitching2/' + split),
]
...
for i, name in enumerate(names):
    ...
    if 'Exp' in dirs[i]:
        eval_path = join(dirs[i].replace('/vis/', '/output/'), 'eval_summary_mask.pkl')
        if isfile(eval_path):

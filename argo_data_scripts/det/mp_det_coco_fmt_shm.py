...
def det_process(opts, in_pipe, out_pipe, frame_buffer):
    try:
        ...
        # get images ready
        frames = np.ctypeslib.as_array(frame_buffer)

        # frames = np.frombuffer(frame_buffer, np.uint8)
        ...
        while 1: # entering daemon mode
            ...
            fidx, t1 = fidx
            # t4 = perf_counter()
            # Using memoryview due to the overhead of RawArray
            # https://stackoverflow.com/questions/37705974/why-are-multiprocessing-sharedctypes-assignments-so-slow
            # converting it to numpy earlier is because memory view does not support slicing multi-dimensional array

            frame = frames[fidx]
            # frame = np.frombuffer(memoryview(frames[fidx]), 'uint8')
            # frame = frame.reshape(img_height, img_width, 3)
            t2 = perf_counter() 
            # print(f'Copy time: {t2-t4}')
            t_send_frame = t2 - t1
            ...
            # res_buffer = result
            out_pipe.send([result, t_send_frame, t3])
